...
execute as @a[tag=player,gamemode=!spectator,tag=!infected,tag=!corrupted,tag=!draconic_stage_3] at @s align xyz positioned ~ ~-1.1 ~ if entity @n[tag=corrupt_block,dx=0,dy=1] run scoreboard players add @s infected 1
execute as @a[tag=player,gamemode=!spectator,tag=!infected,tag=!corrupted,tag=!draconic_stage_3] at @s if score @s infected >= .01 infected run function main:module/corruption/check
